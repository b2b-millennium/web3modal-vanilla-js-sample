...
const contractLib = (function () {
  ...
  async function mintTokens(count, tokenIds) {
    ...
    console.log(count, maxSupply, maxMint, priceInWei, canMint, sessionId, tokenIds, signature);

    if (!await contractB.methods.isApprovedForAll(selectedAccount, Config.contractAddress_A).call()) {
      const approveMethod = contractB.methods.setApprovalForAll(Config.contractAddress_A, true);
